...
class NotificationMatchWorker(private val context: Context, workerParams: WorkerParameters) : Worker(context, workerParams) {
  ...
  companion object {
    fun start(context: Context, match: MatchDomain) {
      ...
      WorkManager.getInstance(context).enqueueUniqueWork(id,
